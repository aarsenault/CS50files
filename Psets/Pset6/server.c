...
int charno(const char* string, char charachter); 
...
void error(unsigned short code)
{
    ...
    // create template array of correct size 
    char body[(strlen(template) - 2 - ((int) log10(code) + 1) - 2 + strlen(phrase)) * 2 + 1];
    
    // will print code in the %i above, and phrase in the %s
...
{   
    ...
    // create new strings
    char* html = "index.html";  // already has '/' 
    char* php = "index.php"; 
    
    // calloc space for new path strings
    char* path_html = malloc( sizeof(char) * (strlen(path) + strlen(html) + 1 ) );
    char* path_php = malloc( sizeof(char) * (strlen(path) + strlen(php) + 1 ) );
    
    // coppies the root path into string
    strcpy(path_html, path_ptr); 
    strcpy(path_php, path_ptr); 
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    if (file == NULL)
    {
        printf("Error opening file in load \n");
        return false; 
    }
    
    size_t size = 0; 
    
    // printf("currently in load \n"); 

    
    // initialize to size of char
    BYTE* buffer = malloc(sizeof(BYTE)); 
    int c = 0; // why does this need to be int?
    
    for (c = fgetc(file); c != EOF; c = fgetc(file))
    {
        // dereferences and stores in that position
        *(buffer + size) = c;    
        size++;
        
        buffer = realloc(buffer, sizeof(BYTE) * ( size + 1)); // increase buffer
    }
    
    // Do not need to null terminate because it is DATA, not a string

    printf("size at End %zu \n", size); 
    *length = size; 
    *content = buffer; 
...
const char* lookup(const char* path)
{
    ...
    // 46 is ASCII for '.'
    char* tag_ptr = strrchr(path, '.');
    
    // 100 chars just to be sure, most are 4
    char termination[100];
    strcpy(termination, tag_ptr ); 
    
    // ERROR CHECKING STATEMENT
    printf("Termination is %s \n", termination); 
    
    if (strcasecmp(termination, ".css" ) == 0)
    {
        return "text/css";
    }
    else if (strcasecmp(termination, ".html" ) == 0)        
    {
        return "text/html";  
    }        
    else if (strcasecmp(termination, ".gif" ) == 0) 
    {
        return "image/gif";
    }    
    else if (strcasecmp(termination, ".ico" ) == 0)
    {
        return "image/x-icon";
    }    
    else if (strcasecmp(termination, ".jpg" ) == 0)
    {
        return "image/jpeg"; 
    }
    else if (strcasecmp(termination, ".js" ) == 0)
    {
        return "text/javascript";
    }    
    else if (strcasecmp(termination, ".php" ) == 0)
    {
        return "text/x-php";
    }       
    else if ( strcasecmp(termination, ".png" ) == 0)
    {
        return "image/png";
    }        
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // initialize buffer
    char requestpath_buffer[LimitRequestLine + 1] = {0}; // Zeroed out
    ...
    printf( "dif is %i \n", dif); 
    printf( "requestpath_buffer[(dif - 1)] is: %c \n", requestpath_buffer[(dif - 1)] );
    printf( "requestpath_buffer[(dif - 2)] is: %c \n", requestpath_buffer[(dif - 2)] );
    printf ("requestpath_buffer[(dif )] is: %c \n", requestpath_buffer[(dif )] );
    ...
    else if (strchr(requestpath_buffer, '\"') != NULL)
    {
        error(400);
        return false; 
    }
    ...
    // Query parsing
    // check for '?' = 63
    char* qmark_ptr = strchr(requestpath_buffer, '?'); 
    ...
    {   
        // strcpy INCLUDES '\0' from requestpath_buffer
        strcpy(abs_path, requestpath_buffer); 
...
/* takes a string ending with '\0' and returns the 
number of specific char in it - CASE DEPENDENT */
...
int charno(const char* string, char charachter)
{
    ...
    // loop over string counting charachters
    while (*ptr != '\0' )
    {
        if (*ptr == charachter)
        {
